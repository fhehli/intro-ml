...
imputer = SimpleImputer(strategy="median") #TODO: try different imputation strategies
...
for target in TESTS:
    ...
    hgbc.fit(X_train, df_training_labels[target])
    df_pred[target] = hgbc.predict_proba(X_test)[:, 1]
...
for target in VITALS:
    ...
    hgbr.fit(X_train, df_training_labels[target])
    df_pred[target] = hgbr.predict(X_test)
